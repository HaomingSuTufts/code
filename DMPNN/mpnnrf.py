...
def load_model(model_path):
    ...
    """
    model = MPNNEncoder(node_in_feats=8, edge_in_feats=4, node_out_feats=300, edge_hidden_feats=256, num_step_message_passing=6)
...
def main(model_path, data_path):
    ...
    save_embeddings = pd.DataFrame(train_embeddings)
    save_embeddings.to_csv(data_path[:-4] + '_train_embeddings.csv', index=False)
    rf = random_forest_regression(train_embeddings, train_data['expt'])
    mean, std = test_model(rf, test_embeddings)


    rmse, nll, cnll = evaluate_result(mean, std, test_data['expt'])

    plot_result(mean, std, test_data['expt'])
    plot_rmse_std_relation(rmse, std)
    plot_rmse_nll_relation(rmse, nll)

    result = pd.DataFrame({'smiles': test_data['smiles'], 'expt': test_data['expt'], 'mean': mean, 'std': std, 'rmse': rmse, 'nll': nll, 'cnll': cnll})
